    ...
    def makeObservation(self, index):
        ...
        for enemy in otherTeam:
            ...
            for teammate in team:
                if (
                    util.manhattanDistance(enemyPos, state.getAgentPosition(teammate))
                    <= SIGHT_RANGE
                ):
                    seen = True
            if not seen:
...
class AgentRules:
    ...
    def applyAction(state, action, agentIndex):
        ...
        if next == nearest:
            ...
            # Change agent type
            agentState.isPacman = [isRed, state.isRed(agentState.configuration)].count(
                True
            ) == 1
            ...
            if agentState.numCarrying > 0 and not agentState.isPacman:
                ...
                for index in range(state.getNumAgents()):
                    ...
                    if index in state.getRedTeamIndices():
                        ...
                        blueCount += agentState.numReturned
                if (
                    redCount >= (TOTAL_FOOD / 2) - MIN_FOOD
                    or blueCount >= (TOTAL_FOOD / 2) - MIN_FOOD
                ):
                    state.data._win = True
    ...
    def consume(position, state, isRed):
        ...
        if state.data.food[x][y]:
            ...
            agents = [
                state.data.agentStates[agentIndex] for agentIndex in teamIndicesFunc()
            ]
            for agent in agents:
        ...
        if agentState.isPacman:
            for index in otherTeam:
                ...
                if ghostPosition == None:
                    continue
                if (
                    manhattanDistance(ghostPosition, agentState.getPosition())
                    <= COLLISION_TOLERANCE
                ):
            ...
            for index in otherTeam:
                ...
                if pacPos == None:
                    continue
                if (
                    manhattanDistance(pacPos, agentState.getPosition())
                    <= COLLISION_TOLERANCE
                ):
                    # award points to the other team for killing Pacmen
...
def readCommand(argv):
  ...
  """
    ...
    parser.add_option("--red-name", help=default("Red team name"), default="Red")
    parser.add_option("--blue-name", help=default("Blue team name"), default="Blue")
    parser.add_option(
        "--redOpts", help=default("Options for red team (e.g. first=keys)"), default=""
    )
    parser.add_option(
        "--blueOpts",
    ...
    args["agents"] = sum(
        [list(el) for el in zip(redAgents, blueAgents)], []
    )  # list of agents
...
def loadAgents(isRed, factory, textgraphics, cmdLineArgs):
    ...
    try:
        ...
        print(
            'Error: The team "' + factory + '" could not be loaded! ', file=sys.stderr
        )
        traceback.print_exc()
...
def runGames(
    ...
    for i in range(numGames):
        ...
        if beQuiet:
            ...
            rules.quiet = False
        g = rules.newGame(
            layout, agents, gameDisplay, length, muteAgents, catchExceptions
        )
        g.run()
        # if not beQuiet:
        games.append(g)
