...
class Maze:
    ...
    def add_wall(self, i, gaps=1, vert=True):
        ...
        if vert:
            ...
            if not 0 in slots:
                if self.root.grid[min(slots) - 1][add_c + i] == E:
                    slots.remove(min(slots))
                if len(slots) <= gaps:
                    return 0
            if not self.root.c - 1 in slots:
                if self.root.grid[max(slots) + 1][add_c + i] == E:
            ...
            self.rooms.append(
                Maze(self.r, self.c - i - 1, (add_r, add_c + i + 1), self.root)
            )
        else:
            gaps = min(self.c, gaps)
            slots = [add_c + x for x in range(self.c)]
            if not 0 in slots:
                if self.root.grid[add_r + i][min(slots) - 1] == E:
                    slots.remove(min(slots))
                if len(slots) <= gaps:
                    return 0
            if not self.root.r - 1 in slots:
                if self.root.grid[add_r + i][max(slots) + 1] == E:
                    slots.remove(max(slots))
            if len(slots) <= gaps:
                return 0
            random.shuffle(slots)
            ...
            self.rooms.append(
                Maze(self.r - i - 1, self.c, (add_r + i + 1, add_c), self.root)
            )
...
def make_with_prison(room, depth, gaps=1, vert=True, min_width=1, gapfactor=0.5):
    ...
    room.rooms.append(
        Maze(room.r, room.c - (2 * p), (add_r, add_c + (2 * p)), room.root)
    )
        ...
        for row in range(1, maze.r - 1):
            for col in range(1 + toskip, int(maze.c / 2) - 1):
                ...
                if maze.grid[row][col] != E:
                    continue
                neighbors = (
                    (maze.grid[row - 1][col] == E)
                    + (maze.grid[row][col - 1] == E)
                    + (maze.grid[row + 1][col] == E)
                    + (maze.grid[row][col + 1] == E)
                )
                if neighbors == 1:
...
def generateMaze(seed=None):
    ...
    skip = make_with_prison(
        maze, depth=0, gaps=3, vert=True, min_width=1, gapfactor=gapfactor
    )
    maze.to_map()
